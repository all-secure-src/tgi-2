...
class OmegaNeoAttention(torch.nn.Module):
    def __init__(
        self,
        prefix: str,
        config,
...
class OmegaNeoLayer(nn.Module):
    def __init__(self, layer_id, config, weights):
        super().__init__()
        prefix = f"model.layers.{layer_id}"
        self.self_attn = OmegaNeoAttention(
            prefix=f"{prefix}.self_attn", config=config, weights=weights
        )
        self.mlp = OmegaNeoMLP(prefix=f"{prefix}.mlp", config=config, weights=weights)
...
class OmegaNeoModel(torch.nn.Module):
...
class OmegaNeoForCausalLM(torch.nn.Module):
    def __init__(self, config, weights):
        super().__init__()

        self.model = OmegaNeoModel(config, weights)
