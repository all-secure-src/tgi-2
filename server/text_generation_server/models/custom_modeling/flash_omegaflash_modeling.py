...
class OmegaFlashRotary(PositionRotaryEmbedding):
    def forward(
        self,
        query: torch.Tensor,
        key: torch.Tensor,
...
class OmegaFlashLayerNorm(nn.Module):
...
class FlashOmegaFlashAttention(torch.nn.Module):
    def __init__(
        self,
        prefix: str,
        config,
        ...
        self.rotary_emb = OmegaFlashRotary.static(
            config=config,
            dim=self.head_size,
        ...
        if self.use_qk_norm:
            self.q_norm = OmegaFlashLayerNorm(
                prefix=f"{prefix}.q_norm",
                weights=weights,
                eps=config.layer_norm_eps,
            )
            self.k_norm = OmegaFlashLayerNorm(
...
class FlashOmegaFlashLayer(nn.Module):
    def __init__(self, layer_id, config, weights):
        super().__init__()
        prefix = f"model.layers.{layer_id}"
        self.self_attn = FlashOmegaFlashAttention(
            prefix=f"{prefix}.self_attn", config=config, weights=weights
        )
        self.mlp = OmegaFlashMLP(prefix=f"{prefix}.mlp", config=config, weights=weights)
...
class FlashOmegaFlashModel(torch.nn.Module):
...
class FlashOmegaFlashForCausalLM(torch.nn.Module):
    def __init__(self, config, weights):
        super().__init__()

        self.model = FlashOmegaFlashModel(config, weights)
        try:
